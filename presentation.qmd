...
The chart presents layoffs across multiple sectors on a single shared timeline. Viewers can immediately see *when* layoffs occurred and *which* industries were affected — a strength especially useful for identifying peaks.

. . .

**✅ Effective Use of Bubble Size**

Size variation gives an instant visual cue about the *magnitude* of layoffs. Larger bubbles draw the eye to major events (e.g., Meta, Amazon), aiding quick pattern recognition.
...
Labeling only the most prominent companies avoids text overload while still providing essential real-world context for the largest layoff events.

. . .

**✅ Clear Chronological Structure**

The timeline structure allows viewers to correlate the onset of job losses with the beginning of interest rate increases — supporting the chart's core narrative.
...
**❌ Severe Overplotting and Occlusion**

Large bubbles (e.g., Meta, Amazon) physically obscure dozens of smaller events in the same time window. Crowded sectors like Big Tech appear as an undifferentiated mass, making it impossible to observe within-sector trends.

. . .

**❌ The "Area" Perception Problem**

Human perception is unreliable when comparing 2D areas (Cleveland & McGill, 1984). Without a size legend, viewers cannot make quantitative judgements — the chart becomes purely impressionistic.
...
Viewers must simultaneously decode *position*, *size*, *colour*, and *timeline* — four visual channels at once. This raises the perceptual effort needed to extract any single insight.

. . .

**❌ Poor Colour Accessibility**

High-saturation neon colours on a black background are visually striking but inaccessible to colour-vision-deficient viewers. Adjacent overlapping bubbles also suffer perceptual interference, further reducing legibility.
...
Each improvement is directly mapped to a specific weakness identified above — ensuring our changes are purposeful and evidence-based.
...
> Separating each sector into its own panel eliminates occlusion entirely. Consistent axes across all panels allow meaningful cross-sector comparison while preserving within-sector trends that were invisible in the original.
...
- Within-sector time trends unreadable
:::
::: {.column width="50%"}
...
- Cross-sector comparison remains possible via aligned axes
:::
::::
...
Layoff activity surges sharply after March 2022. Monthly layoff totals *after* the first rate hike are substantially higher than the pre-hike baseline, providing empirical support for the original chart's narrative.
...
Post-IPO companies account for a disproportionately large share of total layoffs. Publicly listed firms — subject to quarterly earnings pressure — appear to have responded most aggressively to tightening financial conditions.

. . .

**💰 Capital & Scale**

There is a modest positive correlation between funds raised and layoff count. This likely reflects company size rather than recklessness: larger firms employed more people and thus shed more in absolute terms.
